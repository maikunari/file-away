...
if(!class_exists('fileaway_management'))
{
	class fileaway_management
	{
		...
		public function manager()
		{
			...
			// Security fix: Only allow specific actions for unauthenticated users
			$public_actions = array('flightbox', 'bulkdownload');
			if(!is_user_logged_in() && !in_array($action, $public_actions)) {
				return $this->response($this->error(__('Authentication required','file-away')));
			}
		...
		{
			extract($this->pathoptions);
			// CVE-2025-2512 fix: Require authentication for file uploads
			if(!is_user_logged_in()) return $this->error(__('User error','file-away'));
			if(!current_user_can('upload_files')) return $this->error(__('Permission denied','file-away'));
			...
			$dangerous_exts = array('php', 'php3', 'php4', 'php5', 'php7', 'phtml', 'phar', 'phps', 'cgi', 'pl', 'py', 'exe', 'sh', 'bat', 'asp', 'aspx', 'jsp', 'htaccess', 'htpasswd');
			if(in_array(strtolower($check_ext), $dangerous_exts)) return $this->error(__('File Type Error','file-away'));

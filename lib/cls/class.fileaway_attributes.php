...
if(!class_exists('fileaway_attributes'))
{
	class fileaway_attributes
	{
		...
		protected function correct($atts, $ctrl)
		{
			...
			// CVE-2023-0431 fix: Sanitize free-text attributes to prevent XSS
		...
		/**
		 * CVE-2023-0431 fix: Sanitize shortcode attributes to prevent XSS
		 */
		protected function sanitize_atts($atts)
		{
			// Attributes that accept free text and are output in HTML
			$text_atts = array('class', 'name', 'heading', 'searchlabel', 'filenamelabel', 'datelabel', 'customdata',
				'drawerlabel', 'parentlabel', 'capcolumn', 'descolumn', 'uploadlabel', 'placeholder', 'classes');
			foreach($text_atts as $att)
			{
				if(isset($atts[$att]) && $atts[$att])
				{
					$atts[$att] = esc_attr($atts[$att]);
				}
			}
			return $atts;
		}
